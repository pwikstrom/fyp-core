...
def extract_audio(verbose=False):
    ...
    video_files_before = fyp.list_files_in_storage(main_media_storage, prefix=cf["media_storage"]["video_prefix"], include_sub_prefixes=False, suffix=".mp4")
    audio_files_before = fyp.list_files_in_storage(main_media_storage, prefix=join(cf["media_storage"]["video_prefix"],cf["media_storage"]["audio_prefix"]), include_sub_prefixes=False, suffix=".mp3")
    ...
    for filename in video_files_to_process:
        ...
        if exists(output_file):
            remove(output_file)
        fyp.load_blob_from_storage(main_media_storage, filename, prefix=cf["media_storage"]["video_prefix"], dest_dir=fyp.temp_path())
        ...
        fyp.save_blob_to_storage(main_media_storage, basename(output_file), source_dir=fyp.temp_path(), prefix=join(cf['media_storage']["video_prefix"],"extracted_audio"))
        if exists(input_file):
            remove(input_file)
        if exists(output_file):
            remove(output_file)
    ...
    audio_files_after = fyp.list_files_in_storage(main_media_storage, prefix=join(cf["media_storage"]["video_prefix"],cf["media_storage"]["audio_prefix"]), include_sub_prefixes=False, suffix=".mp3")
